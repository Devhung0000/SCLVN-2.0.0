...
export default {
    template: `
        <main class="page-profile" style="max-width: 800px; margin: 40px auto; padding: 20px; font-family: 'Poppins', sans-serif;">
            <div v-if="!store.user" style="text-align: center; color: #a1a1aa; padding: 50px;">
                <h2>Vui lòng đăng nhập để xem và chỉnh sửa Profile!</h2>
            </div>
            ...
            <div v-else style="background: #18181b; border: 1px solid #27272a; border-radius: 12px; padding: 30px; color: white; box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
                ...
                <div style="display: flex; gap: 30px; margin-top: 25px; flex-wrap: wrap;">
                    ...
                    <div style="display: flex; flex-direction: column; align-items: center; gap: 15px;">
                        ...
                        <span style="font-size: 11px; color: #71717a;">Chấp nhận file .PNG / .JPG</span>
                    </div>

                    <!-- Form Input Fields -->
                    <form @submit.prevent="saveProfile" style="flex: 1; min-width: 280px; display: flex; flex-direction: column; gap: 16px;">
                        <div>
                            <label style="font-size: 13px; color: #a1a1aa; display: block; margin-bottom: 6px;">Player Name (Khớp với Leaderboard):</label>
                        ...
                        <div>
                            <label style="font-size: 13px; color: #a1a1aa; display: block; margin-bottom: 6px;">YouTube Channel / Link Video:</label>
                        ...
                        <div>
                            <label style="font-size: 13px; color: #a1a1aa; display: block; margin-bottom: 6px;">Facebook Profile Link:</label>
                        ...
                        <div>
                            <label style="font-size: 13px; color: #a1a1aa; display: block; margin-bottom: 6px;">GDVN Profile Link:</label>
                        ...
                        <div>
                            <label style="font-size: 13px; color: #a1a1aa; display: block; margin-bottom: 6px;">Discord User ID / Tag:</label>
                            <input type="text" v-model="form.discord" placeholder="e.g. 1338170769708159032" style="width: 100%; padding: 10px; border-radius: 6px; border: 1px solid #3f3f46; background: #27272a; color: white; box-sizing: border-box;">
                        </div>
    ...
    methods: {
        async loadInitialData() {
            ...
            // 1. Lấy dữ liệu từ file _players.json làm giá trị mặc định (nếu có)
            try {
                ...
                console.error("Không tải được _players.json", err);
            }

            // 2. Nếu thông tin trên Firestore đã có thì ghi đè dữ liệu mới nhất của user
        ...
        handleFileUpload(e) {
            ...
            reader.onload = (evt) => {
                this.previewAvatar = evt.target.result; // Chuyển file ảnh thành chuỗi Base64
            };
